...
import { LogOut, User as UserIcon, Shield, Bell, X, Eye, Loader as Loader2 } from "lucide-react";
...
export function Profile() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#050505] overflow-y-auto w-full no-scrollbar">
      <div className="flex flex-col items-center mb-6 pt-6 px-6">
        <div className="w-20 h-20 bg-zinc-800 rounded-full mb-3 flex items-center justify-center border border-white/10 shadow-xl relative overflow-hidden">
          {profile?.avatarUrl ? (
            <img src={profile.avatarUrl} alt="Profile" className="w-full h-full object-cover" />
          ) : (
            <UserIcon size={36} className="text-zinc-600" />
          )}
        </div>
        <h2 className="text-xl font-bold text-white tracking-tight">{profile?.displayName || user?.email?.split('@')[0] || 'User'}</h2>
        <p className="text-[11px] text-blue-500 font-mono mt-0.5">@{profile?.username || 'handle'}</p>

        <p className="text-center text-zinc-400 mt-3 text-sm max-w-xs leading-relaxed">{profile?.bio || 'Hey there! I am using Swift Chat.'}</p>
        <button onClick={openEditModal} className="mt-4 bg-white/5 border border-white/10 text-white px-5 py-2 rounded-full text-xs font-bold hover:bg-white/10 active:scale-95 transition-all">Edit Profile</button>
      </div>

      <div className="flex justify-center divide-x divide-white/10 mb-6 border-y border-white/5 py-3 bg-[#0A0A0A]">
      ...
      <div className="px-4 pb-safe">
        <button
          onClick={signOut}
          className="w-full bg-red-500/10 text-red-500 font-semibold rounded-2xl py-3.5 px-4 flex items-center justify-center space-x-2 hover:bg-red-500/20 active:scale-[0.98] transition-all"
        >
          <LogOut size={18} />
          <span>Log Out</span>
        </button>
      ...
      <p className="text-center text-xs text-gray-600 mt-4 pb-4">Swift Chat v1.0.0</p>

      {showEdit && (
        <div className="fixed inset-0 z-50 bg-black/80 flex justify-center items-end sm:items-center p-0 sm:p-4">
          <div className="bg-[#0A0A0A] border border-white/10 w-full max-w-sm rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl max-h-[85vh] overflow-y-auto pb-safe">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-white">Edit Profile</h3>
              <button onClick={() => setShowEdit(false)} className="text-gray-500 hover:text-white p-1"><X size={20} /></button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <button disabled={saving} onClick={handleSaveProfile} className="w-full bg-blue-600 text-white rounded-xl py-3 font-bold mt-2 disabled:opacity-50 hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center gap-2">
...
function ToggleItem({ icon: Icon, label, description, active, onToggle }: { icon: any, label: string, description: string, active: boolean, onToggle: () => void }) {
  return (
    <div onClick={onToggle} className="flex items-center justify-between p-4 bg-[#0A0A0A] hover:bg-white/5 active:bg-white/10 transition-colors cursor-pointer">
