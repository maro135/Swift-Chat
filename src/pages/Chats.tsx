...
import { MessageSquare, Search, X, User as UserIcon, Loader as Loader2 } from "lucide-react";
...
export function Chats() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#050505] px-4 pt-2 relative">
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center px-4">
          ...
          <button
            onClick={() => setShowCreate(true)}
            className="mt-6 bg-blue-600 text-white rounded-full px-6 py-2.5 font-medium hover:bg-blue-700 active:scale-95 transition-all">
            New Chat
          </button>
        </div>
      ) : (
        <div className="space-y-2 pb-4">
          {chats.map((chat, i) => (
            ...
            >
              <div className="w-10 h-10 rounded-full bg-blue-500/20 border border-blue-500/30 flex items-center justify-center text-blue-400 text-xs flex-shrink-0 overflow-hidden">
              ...
              <div className="flex-1 min-w-0 pointer-events-none">
                ...
                <div className="flex items-center gap-1">
                  {chat.lastMessageSenderId === auth.currentUser?.uid && (
                    <span className="text-blue-500 shrink-0"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 7 17l-5-5"/><path d="m22 10-7.5 7.5L13 16"/></svg></span>
                  )}
                  <p className="text-xs text-zinc-500 truncate">{chat.recentMessage || 'Started a new chat'}</p>
                </div>
              </div>
            </motion.div>
          ))}
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 bg-black/80 flex justify-center items-end sm:items-center p-0 sm:p-4">
          <div className="bg-[#0A0A0A] border border-white/10 w-full max-w-sm rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl flex flex-col max-h-[85vh] pb-safe">
            ...
            <div className="space-y-4 flex-1 overflow-y-auto no-scrollbar">
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <button onClick={handleCreateChat} disabled={creating || !targetUsername.trim()} className="bg-blue-600 text-white rounded-xl px-4 font-bold disabled:opacity-50 text-sm active:scale-95 transition-transform">
                    {creating ? <Loader2 size={16} className="animate-spin" /> : 'Find'}
                  </button>
                </div>
                ...
                <div className="mt-4 border-t border-white/5 pt-4">
                  ...
                  <div className="space-y-1">
                    {availableUsers.map(u => (
                      <div onClick={() => startChatWithUser(u)} key={u.id} className="flex items-center gap-3 p-2.5 hover:bg-white/5 active:bg-white/10 rounded-xl cursor-pointer transition-colors">
                        <div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center text-xs overflow-hidden shrink-0">
                          {u.avatarUrl ? <img src={u.avatarUrl} className="w-full h-full object-cover" /> : <UserIcon size={14} className="text-zinc-500" />}
                        </div>
                        <div className="flex flex-col min-w-0">
                          <span className="text-sm font-medium text-white truncate">{u.displayName || u.username}</span>
